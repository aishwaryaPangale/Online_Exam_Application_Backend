...
public class StartTestRepositroy {
	 ...
	 public Map<String, Object> getTestDetails(int testId) {
		 ...
		 String sql = "SELECT t.*, c.course_name, b.batch_name FROM test t JOIN course c ON t.course_id = c.id JOIN batch b ON t.batch_id = b.id " +
		 ...
		 String sql = "SELECT q.* FROM questions q JOIN test_question tq ON q.id = tq.question_id " +
